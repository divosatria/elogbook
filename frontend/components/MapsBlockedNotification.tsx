          ...
          <div className="p-4">
            ...
            {showDetails && (
              <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200 max-h-64 overflow-y-auto">
                <h4 className="font-medium text-slate-800 text-sm mb-3">🔓 Cara Membuka Blokir Maps:</h4>
                <div className="space-y-3 text-xs text-slate-600">
                  <div className="p-3 bg-blue-50 rounded border border-blue-200">
                    <p className="font-bold text-blue-800 mb-2">✨ CARA TERCEPAT (Semua Browser):</p>
                    <ol className="list-decimal list-inside space-y-1 text-blue-700">
                      <li>Cari ikon <strong>🔒</strong> atau <strong>ⓘ</strong> di address bar</li>
                  ...
                  <div>
                    <p className="font-medium text-slate-700 mb-2">🌐 Chrome:</p>
                    <p className="mb-1">• Klik ikon gembok → Site settings → Location → Allow</p>
                    <p className="text-slate-500">• Atau ketik: chrome://settings/content/location</p>
                  </div>
                  
                  <div>
                    <p className="font-medium text-slate-700 mb-2">🦊 Firefox:</p>
                    <p className="mb-1">• Klik ikon shield → Turn off Tracking Protection</p>
                    <p className="text-slate-500">• Atau: Settings → Privacy & Security → Location</p>
                  </div>
                  
                  <div>
                    <p className="font-medium text-slate-700 mb-2">🧭 Safari:</p>
                    <p className="mb-1">• Safari → Preferences → Websites → Location</p>
                    <p className="text-slate-500">• Atau klik gembok → Website Settings</p>
                  </div>
                  
                  <div>
                    <p className="font-medium text-slate-700 mb-2">🔷 Edge:</p>
                    <p className="mb-1">• Klik gembok → Permissions → Location → Allow</p>
                    <p className="text-slate-500">• Atau: Settings → Site permissions → Location</p>
                  </div>
                  
                  <div className="mt-3 p-2 bg-green-50 rounded border border-green-200">
                    <p className="font-medium text-green-700 mb-1">🧪 Test Berhasil:</p>
                    <p className="text-green-600 text-xs">Tekan F12 → Console → ketik:</p>
                    <code className="text-xs bg-green-100 px-1 rounded">navigator.geolocation.getCurrentPosition(console.log)</code>
                  </div>
                  
                  <div className="mt-3 p-2 bg-yellow-50 rounded border border-yellow-200">
                    <p className="font-medium text-yellow-700 mb-1">🆘 Jika Masih Tidak Bisa:</p>
                    <ul className="text-yellow-600 text-xs space-y-1">
                      <li>• Coba browser lain (Chrome/Firefox)</li>
                      <li>• Gunakan VPN (Cloudflare WARP gratis)</li>
                      <li>• Hubungi admin IT untuk whitelist</li>
                      <li>• Sistem tetap berfungsi dengan input manual</li>
                    </ul>
                  </div>
                </div>
              </div>
