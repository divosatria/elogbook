...
import wppData from '@/src/assets/data/wpp_indonesia.json';
...
const CatchPolygonManagement: React.FC = () => {
  ...
  const renderWPP = (map: any, geojsonData: any) => {
    ...
    // Use original coordinates directly (no smoothing — accurate shapes)
  ...
  const loadPolygons = async () => {
    try {
      console.log('🔍 Loading catch polygons...');
      const response = await backendAPI.getCatchPolygons();
      console.log('📡 Catch polygons response:', response);
      ...
      if (response.success && response.data) {
        ...
        console.log(`✅ Loaded ${transformedPolygons.length} catch polygons successfully`);
        ...
        try {
          ...
          if (directResponse.ok) {
            ...
            if (directData.success && directData.data) {
              console.log('✅ Direct catch polygons API call successful, loaded', directData.data.length, 'polygons');
          ...
          console.error('❌ Direct catch polygons API call also failed:', directError);
        }
      }
    } catch (error) {
