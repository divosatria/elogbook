...
const HarborZones: React.FC<HarborZonesProps> = ({ onBack }) => {
  ...
  const loadPOIs = async () => {
    try {
      console.log('🔍 Loading harbor POIs...');
      const response = await backendAPI.getHarborPOIs();
      console.log('📡 Harbor POIs response:', response);
      const poisData = response.data || response || [];
      console.log('📊 Harbor POIs data:', poisData.length, 'POIs loaded');
      setPois(poisData);
    } catch (error) {
      ...
      try {
        ...
        if (directResponse.ok) {
          ...
          if (directData.success && directData.data) {
            console.log('✅ Direct POI API call successful, loaded', directData.data.length, 'POIs');
            setPois(directData.data);
            return;
          }
        }
      } catch (directError) {
        console.error('❌ Direct POI API call also failed:', directError);
      }
  ...
  const loadZones = async () => {
    try {
      setLoading(true);
      console.log('🔍 Loading harbor zones...');
      const response = await backendAPI.getHarborZones();
      console.log('📡 Harbor zones response:', response);
      const zonesData = response.data || response || [];
      console.log('📊 Harbor zones data:', zonesData.length, 'zones loaded');
      setZones(zonesData);
    } catch (error) {
      ...
      try {
        ...
        if (directResponse.ok) {
          const directData = await directResponse.json();
          if (directData.success && directData.data) {
            console.log('✅ Direct API call successful, loaded', directData.data.length, 'zones');
                ...
                <>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                      ...
                      <p className="text-xs text-blue-600 mt-1">
                        ...
                        {polygonPoints.length >= 3 ? "✓ Siap disimpan" : ""}
                      </p>
                    </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-4xl mx-4 h-[80vh] flex flex-col">
            ...
            <div className="p-4 border-t border-slate-200 flex justify-between items-center">
              <div className="text-sm text-slate-600">
                <p>📍 Klik pada peta untuk menambah titik (minimal 3 titik)</p>
