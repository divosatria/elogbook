...
const LeafletMap: React.FC<LeafletMapProps> = ({ vessels, calculateDistance }) => {
  ...
  const poiTypes = {
    harbor_office: { icon: 'ðŸ›ï¸', color: '#3b82f6', label: 'Kantor Pelabuhan' },
    shipping_office: { icon: 'ðŸš¢', color: '#10b981', label: 'Kantor Pelayaran' },
    customs: { icon: 'ðŸ›¡ï¸', color: '#f59e0b', label: 'Bea Cukai' },
    fuel_station: { icon: 'â›½', color: '#ef4444', label: 'SPBU Laut' },
    repair_dock: { icon: 'ðŸ”§', color: '#8b5cf6', label: 'Dok Reparasi' },
    warehouse: { icon: 'ðŸ“¦', color: '#6b7280', label: 'Gudang' },
    lighthouse: { icon: 'ðŸ—¼', color: '#f97316', label: 'Mercusuar' },
    pilot_station: { icon: 'âš“', color: '#06b6d4', label: 'Stasiun Pandu' }
  };
      ...
      const marker = window.L.marker([vesselPosition.lat, vesselPosition.lng], { icon })
        ...
        .bindPopup(`
          <div class="p-3">
            ...
            ${inDanger ? '<p class="text-sm text-red-600 font-bold mb-2">âš ï¸ ZONA BAHAYA</p>' : 
              zoneStatus.inFishingZone ? '<p class="text-sm text-green-600 mb-2">ðŸŽ£ Zona Tangkap</p>' : 
      ...
      const marker = window.L.marker([poi.coordinates.lat, poi.coordinates.lng], { icon })
        ...
        .bindPopup(`
          <div class="p-3 min-w-[200px]">
            ...
            ${poi.contact?.phone ? `<p class="text-xs text-gray-500 flex items-center mb-1"><span class="mr-1">ðŸ“ž</span>${poi.contact.phone}</p>` : ''}
            ${poi.operatingHours ? `<p class="text-xs text-gray-500 flex items-center mb-1"><span class="mr-1">ðŸ•</span>${poi.operatingHours}</p>` : ''}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-slate-500">
                        Pemilik: {vessel.pemilik} â€¢ Status: {vessel.statusPelayaran === 'sailing' ? 'Berlayar' : vessel.statusPelayaran === 'docked' ? 'Berlabuh' : 'Tidak Diketahui'}
                      </div>
                    </div>
                    <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      {vessel.gps?.currentPosition ? 'ðŸ“ GPS' : 'âŒ No GPS'}
                    </div>
                  </div>
                </button>
              ))}
          ...
          <div className="space-y-2 text-xs">
            ...
            <div>
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 bg-blue-600 rounded flex items-center justify-center text-xs">ðŸ›ï¸</div>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          {selectedVessel.gps.currentPosition.heading && (
                            <div className="flex items-center space-x-2">
                              ...
                              <span>Arah: {selectedVessel.gps.currentPosition.heading.toFixed(0)}Â°</span>
                            </div>
                          )}
                          <div className="flex items-center space-x-2">
