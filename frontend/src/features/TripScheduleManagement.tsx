  ...
  const handleCreateTripFromTask = async (task: TripTask) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`âœ… Jadwal Trip berhasil dibuat!\n\nðŸ“‹ Trip ID: ${result.data.id}\nðŸš¢ Kapal: ${vessel.namaKapal}\nðŸ“… Tanggal: ${new Date(task.scheduledDate).toLocaleDateString('id-ID')}\n\nâž¡ï¸ Trip sudah masuk ke tab "Perizinan Trip"\nðŸ“± Nahkoda & ABK akan dapat notifikasi di mobile app\nðŸ“„ Menunggu upload dokumen dari nahkoda`);
        
        // Mark task as completed with note
        await handleCompleteTask(task.id, `âœ… Trip telah dibuat dari jadwal ini.\nðŸ“‹ Trip ID: ${result.data.id}\nðŸ“± Nahkoda & ABK sudah dapat notifikasi tugas.`);
      } else {
        const error = await response.json();
        alert(`âŒ Gagal membuat trip: ${error.message}`);
      }
    } catch (error) {
      console.error('Error creating trip from task:', error);
      alert('âŒ Gagal membuat trip dari jadwal tugas');
    }
  };
